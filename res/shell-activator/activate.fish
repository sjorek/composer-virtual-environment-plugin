...
if test -n "$COMPOSER_VENV"
    ...
    function deactivate  -d "Exit virtual environment and return to normal shell environment"
        ...
        if test "$argv[1]" != "nondestructive"
            set -l hooks "@SHELL_HOOK_DIR@/pre-deactivate.d/"*.{sh,fish}
            if test (count $hooks) -ge 1
                for f in ( ${hooks} )
                    source "$f"
                end
            end
        ...
        if test "$argv[1]" != "nondestructive"
            ...
            set -l hooks "@SHELL_HOOK_DIR@/post-deactivate.d/"*.{sh,fish}
            if test (count $hooks) -ge 1
                for f in ( ${hooks} )
                    source "$f"
                end
    ...
    set -l hooks "@SHELL_HOOK_DIR@/pre-activate.d/"*.{sh,fish}
    if test (count $hooks) -ge 1
        for f in ( ${hooks} )
            source "$f"
        end
    ...
    set -l hooks "@SHELL_HOOK_DIR@/post-activate.d/"*.{sh,fish}
    if test (count $hooks) -ge 1
        for f in ( ${hooks} )
            source "$f"
        end
